...
`textMeal` は `application/json` の `text`、`inputAt`、`mealId` を受け取ります。`inputAt` は UTC offset を含む RFC 3339、`mealId` は同じ自然文食事の再送で変わらない値にします。`Authorization: Bearer <MealRelay token>` を必須とし、本文にユーザー ID は持たせません。自然文に日時表現がない場合は `inputAt` を食事時刻に使い、`昨日の夜` などは `inputAt` を基準に解析します。自然文で朝食・昼食・夕食・間食の区分が明示された場合は Google Health の `mealType` として保持し、時刻だけから食事区分を推測しません。自然文に明示された栄養値は `userInput` 由来の確定値として保持し、それ以外の項目だけを推定値として扱います。

画像・自然文解析には OpenAI API の [`gpt-6-luna`](https://developers.openai.com/api/docs/models/gpt-6-luna) と公式 [`openai`](https://developers.openai.com/api/docs/libraries) SDK を使用します。Responses API と Structured Outputs を一度のリクエストで使用し、画像からは料理ごとの実摂取量を含む表示名を、自然文からは料理内容と相対日時を取得します。どちらも1食全体のカロリー・たんぱく質・炭水化物・脂質を扱います。画像では複数品を1食へ集約し、大皿・作り置き・複数人分は写真全量ではなく1人が実際に摂取したと考えられる量を推定するよう指示します。入力中の文字列や自然文は食事情報としてだけ扱い、命令として扱いません。モデルの reasoning 設定は API の既定値を使用します。
...
共通モデルと送信時刻は Firestore の `meals/{SHA-256(sub + 区切り文字 + mealId)}` に保存します。Google Health には匿名食品の Nutrition Log を 1 食 1 件として登録し、食事時刻から 1 秒の interval、元の UTC offset、選ばれた栄養値、食品名を送ります。同じ `sub` と `mealId` の再送では保存済みの解析結果を再利用し、登録済みの食事は Google Health へ再作成しません。Google Health への POST 結果が確定しない `pending` 状態では、重複作成を避けるため同じ POST を再実行しません。同一 `mealId` の初回要求が並行した場合は、Firestore の解析予約により一方だけを解析し、もう一方には再送可能な 503 を返します。予約には要求ごとの ID を持たせ、期限切れ後に新しい予約を取得した場合でも以前の要求が新しい予約を解放・確定できないようにします。入力内容が異なる同一 ID は 409 で拒否します。同じ ID で撮影時刻が異なる再送は競合エラーになります。Google Health の匿名食品ログは作成後に編集できないため、変更・削除はこの処理の対象外です。[Nutrition Log の公式仕様](https://developers.google.com/health/data-types/nutrition)と[DataPoint の識別子仕様](https://developers.google.com/health/reference/rest/v4/users.dataTypes.dataPoints)に従います。

Google Health の現行仕様では write 時の client-assigned data point ID はサポートされず、POST に指定しても無視されます。そのため create リクエストには data point の `name` を指定せず、Google Health が生成して返した resource `name` を Firestore に保存します。[Google Health の Data Management](https://developers.google.com/health/data-management) が示す server-generated ID の標準フローに従います。同資料が明記する通り、応答を受け取れなかった POST を再実行すると別 ID の重複 record が作成される可能性があるため、結果が確定しない `pending` record では Google Health への POST を再実行しません。
