...
function googleClient(tokens, grantedScopes = `openid email ${GOOGLE_HEALTH_WRITE_SCOPE}`) {
...
test('Android server auth code sends an empty redirect URI and verifies the signed ID token', async () => {
  ...
  }, `openid https://www.googleapis.com/auth/userinfo.email ${GOOGLE_HEALTH_WRITE_SCOPE}`);
...
test('missing granted scope and invalid signed identities are rejected by the real OAuth client', async () => {
  ...
  for (const [idToken, grantedScopes] of [
    ...
    [tamperedSignature, `openid email ${GOOGLE_HEALTH_WRITE_SCOPE}`],
    [signedIdToken({ aud: 'another-client' }), `openid email ${GOOGLE_HEALTH_WRITE_SCOPE}`],
    [signedIdToken({ iss: 'another-issuer' }), `openid email ${GOOGLE_HEALTH_WRITE_SCOPE}`],
    [signedIdToken({ exp: 1 }), `openid email ${GOOGLE_HEALTH_WRITE_SCOPE}`],
    [signedIdToken({ email_verified: false }), `openid email ${GOOGLE_HEALTH_WRITE_SCOPE}`],
  ]) {
